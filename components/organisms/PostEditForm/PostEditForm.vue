...
const {
  ...
  initialValues: {
    title: data2.value?.post.title,
    body: data2.value?.post.body,
    user_id: data2.value?.post.user_id,
  },
});

const onSubmit = handleSubmit(async (values) => {
  try {
    await $fetch(`${runtimeConfig.public.apiUrl}/v1/posts/${route.params.id}`, {
      method: "PATCH",
      headers: {
        "Accept": "application/json",
...
<template>
  <form @submit="onSubmit" class="h-full">
    ...
    <button :disabled="isSubmitting"
      class="block bg-blue-400 rounded-3xl w-48 mx-auto text-white py-2 hover:bg-blue-300">更新する</button>
  </form>
</template>